#' DavisRigAnalysis Function
#'
#' This function processes behavioral data files generated by the Davis Rig apparatus, which are typically stored as .txt files. 
#' The function reads in the content of the files, extracts metadata such as Animal ID and Start Date, and processes the main data section to compute summary statistics.
#' It calculates the average number of licks for each unique concentration-stimulus combination and the total number of licks across all trials. 
...
#'   - Dynamic columns representing the average number of licks for each unique concentration-stimulus combination.
#' 
#' The function also saves the result to the global environment as 'result'.
#' 
#' @examples
#' # Process a single file
#' AverageLicksPerStim("C:/DavisData/0719test.ms8.txt")
#' 
#' # Process all files in a directory
#' AverageLicksPerStim("C:/DavisData/")
#' 
#' @import dplyr
#' 
#' @export
...
AverageLicksPerStim <- function(path) {
  ...
  process_file <- function(file_path) {
    ...
    # Calculate the average number of LICKS for each unique CONC_STIM combination where LICKS > 0
    avg_licks <- temp_data %>%
      filter(LICKS > 0) %>%
      group_by(CONC_STIM) %>%
      summarise(AVERAGE_LICKS = mean(LICKS, na.rm = TRUE))
    ...
    result <- data.frame(
      ...
      t(avg_licks$AVERAGE_LICKS)
    )
    
    # Set column names
    colnames(result) <- c("Animal_id", "Date", "Num_trials", "Total_Licks", as.character(avg_licks$CONC_STIM))
  ...
  assign("result", combined_results, envir = .GlobalEnv)
